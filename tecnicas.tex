...
\section{Dublês de Teste (EM CONSTRUÇÃO)} % (fold)
...
\begin{itemize}
\item
Objetos \textbf{\textit{Dummy}} geralmente é utilizado apenas para preencher uma lista de parâmetros e nunca é realmente usado.

\item
Objetos \textbf{\textit{Fake}} é utilizado para substituir uma funcionalidade real. Geralmente, o \textit{fake} implementa a mesma funcionalidade, porém de uma maneira muito mais simples. Um exemplo típico é uma base de dados em memória.

\item
Os \textbf{\textit{Stub}} provê respostas prontas para chamadas feitas durante os testes, geralmente não respondendo a qualquer chamada diferente
das pré-definidas.

\item
Os \textbf{\textit{Spy}} é um \textit{stub} que também grava algumas informações baseadas em como ele é chamado. Um exemplo pode ser um serviço de email que grava quantas mensagens foram enviadas.
...
\citeonline{MocksArentStubs} define duas faces da utilização de TDD com dublês, onde o divisor é \textbf{quando} utilizar um dublê:
...
\subsection{Problemas com o uso excessivo de \textit{mocks} e \textit{stubs}} % (fold)
\label{sub:problemas_do_mock_e_stub}

De modo geral, a utilização de dublês de teste é benéfica para o projeto e extremamente útil. Porém, seu uso excessivo pode causar um problema de acoplamento entre o teste e a implementação.

\subsubsection{Contextualizando o \textit{stub}} % (fold)
\label{ssub:contextualizando_o_stub}

Um exemplo da utilização simples e eficiente do \textit{stub} foi mostrado na Seção \ref{sub:testes_de_integracao} sobre testes de integração. Naquele exemplo, foi feito um \textit{stub} do método \texttt{can\_pygmentize?} que faz uma consulta ao sistema operacional para verificar se uma determinada dependência de sistema está instalada. Desta forma, o teste ficou isolado, não importando se a máquina em que o teste for executado tem ou não a dependência instalada, sendo possível simular as duas situações $-$ logicamente, uma maquina não pode, ao mesmo tempo, ter e não ter uma dependência instalada.
...
% subsubsection contextualizando_o_stub (end)

\subsubsection{Contextualizando o \textit{mock}} % (fold)
\label{ssub:contextualizando_o_mock}

Um exemplo da utilização do \textit{mock} é apresentado no Código \ref{code:mock_spec}, onde é mostrado um método de \textit{controller} que, semelhante ao Código \ref{code:todo} apresentado na Seção \ref{sub:a_ubiquidade_do_tdd}, é chamado via Ajax e implementa a atualização dos responsáveis por uma tarefa diretamente do kanban do projeto. Além atualizar os responsáveis pela tarefa tarefa na base de dados, este método captura, trata e envia informações para que a interface do kanban seja atualizada. Este é outro caso onde um experimento foi para produção, sem testes e com um código \textbf{extremamente ruim}. Sendo assim, para este caso não foi utilizado TDD para criar os testes, os testes foram escritos após a implementação.
...
O Código \ref{code:mock_spec_mockista} apresenta o teste para o método \texttt{update\_assignees} com a utilização de \textit{mock}. Como o teste quer validar a funcionalidade para mais de um responsável (\textit{assignees}), são criados dos objetos mockados da classe \textit{Contributor}. Também é necessário fazer um \textit{mock} de uma tarefa. Com esses 3 objetos mockados, está feita a \textit{setup} para o teste. Em seguida são feias as verificações das chamadas de métodos sobre os objetos mockados. Por fim, é feita a chamada ao método do \textit{controller} e é verificado se a resposta do método está de acordo com as expectativas.

\begin{mycode}{rspec}%
{Código do método que atualiza os responsáveis por uma tarefa assincronamente}{code:mock_spec_mockista}
...
O problema do mock está mas verificações de chamada à métodos dos objetos mockados. Pose-se perceber que está se testando \textbf{como} a funcionalidade está sendo implementada e não \textbf{o que} a funcionalidade faz \cite{UnitForAReason}. Com isso, o acoplamento entre o teste e a implementação torna-se extremamente grande, ainda maior que os \textit{stubs}. Qualquer refatoração mínima feita na implementação, mesmo que não interfira no seu comportamento, pode fazer com que os testes quebrem.

Para fazer uma comparação, no Código \ref{code:mock_spec_tradicional} é apresentado o teste para o mesmo exemplo, porém utilizando objetos reais. Note que desta vez foi preciso criar um objeto \textit{Project} para ligar ao objeto \textit{task}, pois quando são utilizados objetos reais, todas as validações são executadas, e uma tarefa obrigatoriamente deve estar ligada a um projeto.

Percebe-se que o teste sem o uso do \textit{mock} fica mais simples e menos acoplado à implementação. Contudo, o teste usando \textit{mock} ganhou um isolamento maior, e os rodarem com uma velocidade 58\% maior do que o teste sem \textit{mock}.

Uma outra questão é sobre a integração. Quando não se utiliza dublês, está se testando a integração entre as classes.

\begin{mycode}{rspec}%
{Código do método que atualiza os responsáveis por uma tarefa assincronamente}{code:mock_spec_tradicional}
# spec/controllers/boards_controller_spec.rb
describe BoardsController do
  context "should update task assignees" do
    it "with more than one assignee" do
...
\citeonline{ArtOfAgileDevelopment} alerta sobre o perigo de utilizar o mock:
...
% subsubsection contextualizando_o_mock (end)

% subsection problemas_do_mock_e_stub (end)
