...
\subsection{Quando o mock é demais, o desenvolvedor desconfia} % (fold)
\label{sub:quando_o_mock_demais_o_desenvolvedor_desconfia}
...
Contudo, a utilização desta técnica deve ser feita com cautela. Quando se utiliza \textit{mock} e/ou \textit{stubs} em demasia, acaba se testando \textbf{como} está sendo feito e não \textbf{o que} \cite{UnitForAReason}. Dessa forma, o acoplamento entre o teste e a implementação em si acaba sendo um grade problema.

Este problema ficou conhecido na comunidade como estilo \textit{mockist} (``mockista", em português) de fazer TDD.
...
% subsection quando_o_mock_demais_o_desenvolvedor_desconfia (end)
