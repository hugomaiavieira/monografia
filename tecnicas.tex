...
\subsection{A descrença e a surpresa} % (fold)
\label{sub:a_descrenca_e_a_surpresa}

Muitos são os relatos de quem começou a utilizar TDD ficou surpreso, pois antes eram céticos e descrentes em relação à técnica. O motivo disso é que os desenvolvedores, principalmente os que já desenvolvem a algum tempo, acreditam que testar não seja parte do seu trabalho, mas sim do pessoal de qualidade. TODO: Rodrigo, o cara do livro ``Emergent Design"\ fala isso. Acha que vale a pena acham mais algumas referências e falar sobre o assunto?

% subsection a_descrenca_e_a_surpresa (end)

\subsection{Usar TDD sempre?} % (fold)
\label{sec:usar_tdd_sempre_}

Existe um debate, algumas vezes fervoroso \cite{TDDGiveMeABreak}, sobre a utilização do TDD em qualquer situação.

\citeonline{TDDGiveMeABreak} diz:

\begin{citacao}
Fazer testes uma parte central do processo porque eles são úteis para os desenvolvedores? Muito bom. Ditar um fluxo de trabalho para os desenvolvedores que funciona em alguns casos como O Único Caminho Verdadeiro: ridículo.

Testar é ajudar os desenvolvedores, e reconhecendo que o teste automatizado são para benefício para os desenvolvedores, ao invés de \textit{cargo-culting}\footnote{O termo cargo-culting é utilizando quando um programador não qualificados ou iniciante (ou não experiência com o problema em mãos) copia um código de um lugar e colá-o em outro, com pouco ou nenhum entendimento de como o código funciona, ou se é realmente exigido na sua nova posição \cite{CargoCulting}.} um fluxo de trabalho e decretando que ``um tamanho de sapato serve para todos os tipos de pés".
\end{citacao}

Já \citeonline{ChromaticTDD} diz:
...
Acho que o tdd deva ser utilizando quando o código irá para produção. Isso acredito ser invariável. Durante o desenvolvimento do kanban-roots, muitas vezes escrevi código para testar um conceito, ou

Lembro-me de dois casos em que ocorreu um bug em produção que quando fui ver era um código que tinha um comentário ``TODO: test it!". Atualmente, existem 13 comentários como esse espalhados no código do kanban-roots. Isso mostra que se os testes não forem feitos antes para um código que irá entrar em produção, escrevê-lo depois acaba sendo um fardo, ignorado até que um problema ocorra. O autor, assim como \citeonline{}, acredita que para um código que irá para produção, a utilização do TDD é essencial. No entanto, para um código é apenas prova de conceito ou experimento, que serão eliminados, a utilização da técnica não é invariavelmente necessária, até porque esse código não será incluído na base.

\citeonline{LegacyCode} define código legado como código sem testes.

\citeonline{CleanCode} define As Três Leis do TDD, onde a primeira delas é ``Você não pode escrever código de produção até que você tenha escrito um teste de unidade com falha.".

Um exemplo é o código \ref{code:gravatar} que captura a foto do contribuidor do projeto. Esta imagem é capturada de um serviço, externo à aplicação, chamado Gravatar\footnote{Serviço que permite a associação de um avatar (imagem) à uma endereço de email. Mais informações em \url{http://gravatar.com}}. Este foi um típico caso onde um experimento foi para produção e neste caso é pior, pois envolve um serviço externo. E se o Gravatar modificar sua API? E se o Gravatar estiver indisponível? Um teste de integração, que é indispensável nesse caso, foi totalmente negligenciado.
...
Existem os que dizem ser equivocado utilizar TDD neste contexto (http://www.thekua.com/atwork/2008/02/if-you-do-test-driven-development-all-the-time-youre-doing-something-wrong/), mas existem os que discordam (http://spin.atomicobject.com/2011/08/24/when-is-it-ok-to-not-tdd/). TODO: Rodrigo, oq eu acha de abordar esse assunto? Eu defenderia que acredito sem mais importante ainda utilizar quando não se tem certeza das coisas, afinal, ao experimentar é quando você mais precisa dos testes.
