...
Destes cinco tipos de dublês, apenas dois serão abordados, sendo eles \textit{mock} e \textit{stub}, por serem mais amplamente utilizados.

\subsection{Problemas com o uso excessivo de \textit{mock} e \textit{stub}} % (fold)
\label{sub:problemas_do_mock_e_stub}

De modo geral, a utilização de dublês de teste é benéfica para o projeto e extremamente útil. Um exemplo da utilização do \textit{stub} foi mostrado na Seção \ref{sub:testes_de_integracao} sobre testes de integração.
...
Contudo, a utilização desta técnica deve ser feita com cautela. Quando se utiliza \textit{stubs} em demasia, cria-se um acoplamento muito grande entre teste e implementação. Por exemplo, se na implementação do método \texttt{count\_points} for preciso fazer uso de mais algum atributo de \textit{Task} (para uma refatoração, por exemplo), o \textit{stub} deverá possuir esse atributo também, fazendo com que o teste tenha que ser alterado. Isso vai contra uma das premissas da refatoração de que se o comportamento do método não é modificado, seu teste deve permanecer o mesmo e continuar passando após ser feita a alteração na implementação.

É importante notar a diferença entre um \textit{stub} e as estratégias de \textit{fixture replacement}\footnote{\textit{Fixture replacement} é uma técnica que tem como objetivo separar a inicialização do teste de sua codificação em si, dando a possibilidade de se criar objetos com estados pré-definidos que possam ser reutilizados em mais de um teste.} baseadas em instâncias. Nestas estratégias o \textit{fixture} é um objeto real da classe. Assim, um objeto criado via \textit{fixture replacement} de instância fica sujeito a detalhes de sua implementação da classe real e, dessa forma, não faz o isolamento que um \textit{stub} em seu lugar faria, pois o stub simplesmente simula uma instância, portanto, não fica sujeito comportamentos inesperados.


Já ao utilizar \textit{mocks} abundantemente, acaba se testando \textbf{como} está sendo feito e não \textbf{o que} \cite{UnitForAReason}. Dessa forma, o acoplamento entre o teste e a implementação em si acaba sendo um grande problema.

O estilo de desenvolvimento onde o desenvolvedor faz uso intenso de \textit{mocks} ficou conhecido na comunidade como estilo \textit{mockist} (``mockista", em português) de fazer TDD e seu oposto ficou conhecido como estilo tradicional \cite{MocksArentStubs}.

Um exemplo deste problema é apresentado no Código \ref{}
