...
Nos dois únicos casos em que ocorreu um \textit{bug} no \textit{kanban-roots} em produção, ao analisar o código em busca da solução foi encontrado um comentário ``TODO: test it!"\ nos métodos onde os \textit{bugs} ocorreram. Atualmente, existem 13 comentários como esse espalhados no código do \textit{kanban-roots}. Isso, de certa forma, comprava que caso os testes não sejam escritos antes, para um código que irá entrar em produção, escrevê-lo depois acaba sendo um fardo, e isso é ignorado até que um problema ocorra.
...
\citeonline{LegacyCode} define que código legado é código sem testes. Deste modo, assim como \citeonline{CleanCode}, acreditamos que para um código ``que irá para produção" a utilização do TDD é essencial, pois do contrário, se está criando código legado.

No entanto, para um código é apenas prova de conceito ou experimento, que serão eliminados, a utilização da técnica não é invariavelmente necessária – apesar do TDD também ser bem útil em alguns desses casos.
