...
O desenvolvimento de software é o ato de elaborar e implementar um sistema computacional que transforme as necessidades de um usuário ou de um mercado em um produto de software. É uma atividade essencialmente intelectual e criativa, exigindo a manipulação de uma gama muito grande de conhecimentos e informações. Além disso, os desenvolvedores de um software se preocupam com o conteúdo e estrutura, além de deverem se preocupar também com o comportamento fazendo com o que o desenvolvimento de software seja uma atividade complexa, devendo ser realizada por especialistas munidos de técnicas que os auxiliem da melhor maneira possível.

Em 2001 um grupo de dezessete especialistas, reconhecidos pela comunidade como grandes nomes do desenvolvimento software, se reuniu para discutir sobre um conjunto de métodos específicos que vinham surgindo e decidiram usar o termo Agilismo para descrever essa nova geração de métodos \cite{AgileStory}. Na mesma reunião, eles também escreveram o Manifesto Ágil \cite{AgileManifesto}, delineando um conjunto de valores e princípios que, em resumo, trilham um caminho para a eliminação de documentação e processos considerados desnecessários, buscando a simplicidade, com foco na geração de valor e proximidade com o cliente, além de possibilitar respostas rápidas e eficazes às mudanças. Desde então, os métodos ágeis vêm ganhando projeção e importância no cenário da produção de software.
...
As metodologias tradicionais de desenvolvimento de software sugerem uma abordagem sistemática e sequencial, que começa com a especificação dos requisitos pelo cliente e progride ao longo do planejamento, modelagem, construção, e implantação, culminando na manutenção progressiva do software acabado \cite{Pressman}.

Além disso, nas metodologias tradicionais, existe uma busca pelo determinismo, que ganha força ao ser amparado pelo famosa curva de custos de correção do software por tempo transcorrido desde o início do processo \cite{Boehm}, mostrada na Figura \ref{img:custo-cascata}.
...
Esta busca pelo determinismo fez com que se pensasse, de forma equivocada, que ao tentar com bastante afinco, consegue-se antecipar todo o conjunto de requisitos e reduzir os custos eliminando as mudanças \cite{TheBusinessOfInnovation}.

Já o agilismo tem como uma de suas premissas que os custos de alterações sejam praticamente lineares ao longo do tempo, independente do ponto em que esteja, fazendo com que a curva de custo de alterações seja semelhante à apresentada na figura \ref{img:custo-agile}, em oposição à curva de custos de Boehm.
...
É importante ressaltar que as histórias são pontuadas pelos desenvolvedores de acordo com o esforço necessário para desenvolvê-las. Essa pontuação serve para medir a velocidade da equipe de desenvolvimento, sendo esta velocidade a soma do número de pontos das histórias desenvolvidas na iteração. Na iteração seguinte, a equipe só pode oferecer a quantidade de pontos conquistados na iteração anterior, ou seja, a velocidade da iteração anterior. Desta forma, o cliente sabe quantos pontos a equipe pode oferecer em uma iteração, servindo de parâmetro para o cliente priorizar as histórias que devem ser desenvolvidas primeiramente. Se uma história necessita de muito esforço para ser desenvolvida e não gera tanto valor para o cliente naquele momento, ele pode deixar esta para depois $-$ muitas vezes acabam nem sendo desenvolvidas, pois tem um custo benefício muito baixo para o cliente.

Mas para que tudo isso seja possível, os métodos ágeis contam com um conjunto de técnicas para dar suporte a seu caráter iterativo e incremental, sendo algumas destas técnicas utilizadas para a realização do teste de software.


\section{Teste de software}
\label{sec:teste_de_software}

O teste de software é considerado por \citeonline{Sommerville} uma técnica dinâmica de verificação e validação, onde é executada uma implementação do software com dados de teste e se examina as saídas do software e seu comportamento operacional para verificar se seu desempenho está conforme necessário.
...
A maneira como o teste de software é aplicado varia de acordo com a metodologia utilizada no desenvolvimento. Tradicionalmente o teste de software é feito somente após o desenvolvimento ter sido concluído \cite{TesteSoftware, Pressman}. Já nos métodos ágeis um sentido oposto é seguido \cite{ArtOfAgileDevelopment}, sendo os testes feitos antes de qualquer linha de código de implementação. Para isso são utilizadas técnicas como \textit{Test-Driven Development} (abordada na Seção \ref{sub:tdd}) e \textit{Behaviour-Driven Development} (abordada na Seção \ref{sub:bdd}).
...
No código \ref{code:acceptance} são apresentados exemplos de teste de aceitação utilizando BDD com a escrita em texto plano, onde os testes são escritos em linguagem natural. O primeiro cenário testa a funcionalidade de clicar e arrastar as tarefas de uma posição para outra no kanban. Já o segundo cenário testa a funcionalidade de limpar a divisão \textit{Done} do kanban, ou seja, retirar do quadro todas as tarefas concluídas.
...
Todos os testes devem ser executados e o novo teste deve falhar pela razão esperada: a funcionalidade associada ao novo teste não foi desenvolvida. Isto aumenta a confiança que se está testando a coisa certa.
...
\citeonline{DDD} diz que uma \textit{ubiquitous language} é uma linguagem comum a todo o projeto, cobrindo toda a cadeia de comunicação, desde conversas entre o cliente e o analista de negócios até conversas internas da equipe de desenvolvimento, chegando aos termos utilizados no código e mesmo nas estruturas de banco de dados.
...
\begin{enumerate}
\item \textbf{Adicionar um novo teste de aceitação com foco em um cenário}

Cada ciclo se inicia com a criação de um novo teste de aceitação, tendo como foco um novo cenário que descreve um determinado comportamento de uma funcionalidade do sistema. Para fazer um paralelo com os métodos tradicionais de desenvolvimento de software, pode-se ver os cenários como casos de uso.

\citeonline{BDDRodrigo} define que, em BDD, o foco do desenvolvedor deve estar em um único cenário por vez, e os benefícios dessa abordagem:
...
Assim como no \hyperref[ssub:ciclo_tdd]{ciclo TDD}, todos os testes devem ser executados e o novo teste deve falhar pela razão esperada: a funcionalidade ainda não foi desenvolvida.
...
Quando se escreve um teste onde não se pode/escolhe usar componentes reais, pode-se substitui-los pelos Dublês de Teste que oferecem uma maneira de isolar as dependências ao criar os testes, permitindo a utilização de componentes fictícios para cumprir os papéis de componentes reais. Com isso, elimina-se complexidade do código dos testes, pois o código de implementação dos objetos é mantido pequeno e com baixo acoplamento.
...
\item
\textbf{\textit{Stub}} provê respostas prontas para chamadas feitas durante os testes, geralmente não respondendo a qualquer chamada diferente das pré-definidas.
